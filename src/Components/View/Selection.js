...
const Selection = ({ isAnalysis = false, onSubmit }) => {
  ...
  const queryFields = [
    { url: 'study', label: 'studies', header: 'Studies', selected: selectedStudies, setSelected: setSelectedStudies },
    { url: 'assay', label: 'assays', header: 'Assays', selected: selectedAssays, setSelected: setSelectedAssays },
    { url: 'dna', label: 'DNAs', header: 'DNA', selected: selectedDna, setSelected: setSelectedDna },
  ]
  ...
  const plotOptionsFields = [
    { name: 'Normalize', options: ['Temporal Mean', 'Mean/std', 'Min/Max', 'None'], selected: normalize, setSelected: setNormalize, defaultValue: 'None' },
    { name: 'Tabs', options: ['Study', 'Assay', 'DNA', 'Media', 'Strain', 'Inducer', 'Name'], selected: tabs, setSelected: setTabs, defaultValue: 'Study' },
    { name: 'Subplots', options: ['Study', 'Assay', 'DNA', 'Media', 'Strain', 'Inducer', 'Name'], selected: subplots, setSelected: setSubplots, defaultValue: 'Name' },
    { name: 'Lines/Markers', options: ['Study', 'Assay', 'DNA', 'Media', 'Strain', 'Inducer', 'Name'], selected: markers, setSelected: setMarkers, defaultValue: 'DNA' },
    { name: 'Plot', options: ['Mean', 'Mean +/- std', 'All data points'], selected: plot, setSelected: setPlot, defaultValue: 'Mean +/- std' },
  ]
  ...
  return (
    <Layout>
          ...
          <Collapse.Panel header="Query" key="1">
            <Collapse>
              {queryFields.map(field => (
                <Collapse.Panel key={field.label} header={field.header} extra={renderClear(field.selected, field.setSelected)}>
      ...
      <Layout.Footer style={{ padding: 0 }}>
        <Button type="primary" onClick={onPlot} block >Plot</Button>
      </Layout.Footer>
    </Layout>
  )
}
